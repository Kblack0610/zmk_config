 ...
 * On peripheral side of split keyboard, shows simplified info
 */
...
/* BLE profile APIs only available on central side */
#if !IS_ENABLED(CONFIG_ZMK_SPLIT) || IS_ENABLED(CONFIG_ZMK_SPLIT_ROLE_CENTRAL)
#include <zmk/events/ble_active_profile_changed.h>
#include <zmk/events/endpoint_changed.h>
#include <zmk/endpoints.h>
#include <zmk/ble.h>
#define HAS_BLE_PROFILE_APIS 1
#else
#define HAS_BLE_PROFILE_APIS 0
#endif
...
int zmk_widget_status_init(struct zmk_widget_status *widget, lv_obj_t *parent) {
...
#if HAS_BLE_PROFILE_APIS
    /* BT Profile - Large centered text (central only) */
    ...
    /* Connection Status (central only) */
...
#else
    /* Peripheral side - show "RIGHT" label */
    widget->bt_label = lv_label_create(widget->obj);
    lv_obj_align(widget->bt_label, LV_ALIGN_TOP_MID, 0, 5);
    lv_label_set_text(widget->bt_label, "RIGHT");

    widget->conn_label = NULL;
#endif

    /* Battery Level - available on both sides */
